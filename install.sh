...
declare -a FILES_TO_SYMLINK=$(find . -type f -maxdepth 1 \
    ...
    -not -name .gitignore \
    | sed -e 's|//|/|' | sed -e 's|./.|.|')
FILES_TO_SYMLINK="$FILES_TO_SYMLINK .asdfrc .alacritty.yml .Brewfile .config/karabiner .config/nvim .ctags.d .default-gems .docker/config.json .gnupg/gpg.conf .gnupg/gpg-agent.conf .hammerspoon .jira.d .tmux .tool-versions .um .vim"
