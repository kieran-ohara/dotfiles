...
declare -a FILES_TO_SYMLINK=$(find . -type f -maxdepth 1 \
    ...
    -not -name .gitignore \
    | sed -e 's|//|/|' | sed -e 's|./.|.|')
FILES_TO_SYMLINK="$FILES_TO_SYMLINK .asdfrc .Brewfile .config/karabiner .ctags.d .default-gems .docker/config.json .hammerspoon .gnupg/gpg.conf .gnupg/gpg-agent.conf .tmux tool-versions .um .vim"
